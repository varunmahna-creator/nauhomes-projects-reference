                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            {(entry.videos || []).map((vid, vidIdx) => (
                              <div key={vidIdx} className="group relative aspect-video rounded overflow-hidden bg-gray-100 border border-gray-200">
                                ...
                                <button onClick={() => removeTimelineVideo(idx, vidIdx)} className="absolute top-1 right-1 hidden group-hover:flex h-6 w-6 items-center justify-center rounded-full bg-red-500 text-white cursor-pointer"><X className="h-3 w-3" /></button>
                              </div>
                            ))}
                          </div>
                        )}
